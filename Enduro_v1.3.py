...
class Game:
    ...
    def __init__(self):
        ...
        # Load
        pyxel.load("assets/carro1.3.pyxres")
    ...
    def som_abertura(self):
        ...
        snd = 0 # Som registrado no editor
    ...
    def reiniciar(self):
        global score, morto, ativo, p_x, p_y, velocidade, bateu, carros
        global bordas, record, gravar
        ...
        records = open('assets/records.txt', 'r')
        for linha in records:
            points = linha.split()
            if int(points[0]) >= record:
                record = int(points[0])
        records.close()
    ...
    def record(self):
        global record, gravar

        # Sobreescreve com o novo record com o valor de score        
        if morto and score > record and gravar:
            hoje = datetime.datetime.now()
            date = (hoje.strftime('%d/%m/%y -- %H:%M:%S'))
            file = open('assets/records.txt', 'a')
            file.write(str(score)+ ' points in ' + str(date) + '\n')
            file.close()
            gravar = False
        ...
        if limpa_record:
            record = 0
            file = open('assets/records.txt', 'w')
    ...
    def desenha_player(self):
        ...
        v = 0 # y na imagem do pyxeleditor
